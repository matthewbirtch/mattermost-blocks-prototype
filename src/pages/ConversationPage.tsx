...
function SelectionChips({ items }: { items: string[] }) {
  return (
    <div className={styles.selectionChips}>
      {items.map(item => (
        <span key={item} className={styles.selectionChip}>{item}</span>
      ))}
    </div>
  )
}
...
function SelectionList({ items }: { items: string[] }) {
  return (
    <ul className={styles.selectionList}>
      {items.map(item => (
        <li key={item} className={styles.selectionListItem}>{item}</li>
      ))}
    </ul>
  )
}
...
export default function ConversationPage() {
  ...
  const cadenceLabel = cadence !== null ? CADENCE_LABELS[cadence - 1] : 'Weekly'
  ...
  function restart() {
    ...
    setTool(null)
    setCadence(null)
    setMembers({ ethan: true, danielle: false, isabella: true, arjun: true, leila: false })
  }
  ...
  return (
    <div className={['page', styles.conversationPage].join(' ')}>
      ...
      {phase >= 1 && (
        <Msg>
          <Post avatarSrc={userDavidL} avatarAlt="David Liang" username="David Liang" timestamp="just now">
            <p style={msgText}>Hey Matty, can you help me set up an on-call rotation for the mobile team?</p>
          </Post>
        </Msg>
      )}

      {/* 2 ── Matty: typing ──────────────────────────────────────────────── */}
      {phase === 2 && <Msg><TypingPost /></Msg>}

      {/* 3 ── Matty: tool picker ─────────────────────────────────────────── */}
      {phase >= 3 && (
        <Msg>
          <Post avatarSrc={avatarMatty} avatarAlt="Matty" username="Matty" timestamp="just now" isBot botLabel="Agent">
            <p style={msgText}>Sure! Let me get that set up. Which scheduling tool should I configure this in?</p>
            {(phase === 3 || dismissing === 3) && (
              <div className={widgetCls(3)}>
                <MessageAttachment
                  header={{ title: 'Select Scheduling Tool', chip: { label: 'AWAITING INPUT', variant: 'warning' } }}
                  actions={[
                    { label: 'Confirm', variant: 'primary',   onClick: () => { if (tool !== null) dismissAndAdvance(3, 4) } },
                    { label: 'Cancel',  variant: 'secondary' },
                  ]}
                >
                  <div style={pickerGroup}>
                    <SingleSelectPicker index={1} label="PagerDuty" description="Enterprise on-call scheduling and incident response"  selected={tool === 1} onClick={() => setTool(1)} />
                    <SingleSelectPicker index={2} label="OpsGenie"  description="Flexible alerting and on-call management by Atlassian" selected={tool === 2} onClick={() => setTool(2)} />
                    <SingleSelectPicker index={3} label="Linear"    description="Project-based on-call for engineering-first teams"     selected={tool === 3} onClick={() => setTool(3)} border={false} />
                  </div>
                </MessageAttachment>
              </div>
            )}
      ...
      {phase >= 6 && (
        <Msg>
          <Post avatarSrc={avatarMatty} avatarAlt="Matty" username="Matty" timestamp="just now" isBot botLabel="Agent">
            <p style={msgText}>
              {toolLabel}, got it. Who should be on the rotation? I've suggested a few people based on the #mobile-team channel.
            </p>
            {(phase === 6 || dismissing === 6) && (
              <div className={widgetCls(6)}>
                <MessageAttachment
                  header={{ title: 'Add Team Members', chip: { label: 'AWAITING INPUT', variant: 'warning' } }}
                  actions={[
                    { label: 'Add to rotation', variant: 'primary',   onClick: () => dismissAndAdvance(6, 7) },
                    { label: 'Skip',            variant: 'secondary' },
                  ]}
                >
                  <div style={pickerGroup}>
                    <MultiSelectPicker label="Ethan Brooks"   checked={members.ethan}    onChange={e => setMembers(s => ({ ...s, ethan:    e.target.checked }))} />
                    <MultiSelectPicker label="Danielle Okoro" checked={members.danielle} onChange={e => setMembers(s => ({ ...s, danielle: e.target.checked }))} />
                    <MultiSelectPicker label="Isabella Cruz"  checked={members.isabella} onChange={e => setMembers(s => ({ ...s, isabella: e.target.checked }))} />
                    <MultiSelectPicker label="Arjun Patel"    checked={members.arjun}    onChange={e => setMembers(s => ({ ...s, arjun:    e.target.checked }))} />
                    <MultiSelectPicker label="Leila Haddad"   checked={members.leila}    onChange={e => setMembers(s => ({ ...s, leila:    e.target.checked }))} border={false} />
                  </div>
                </MessageAttachment>
              </div>
            )}
      ...
      {phase >= 7 && (
        <Msg>
          <Post avatarSrc={userDavidL} avatarAlt="David Liang" username="David Liang" timestamp="just now">
            <SelectionList items={selectedNames.length > 0 ? selectedNames : ['Ethan Brooks', 'Isabella Cruz', 'Arjun Patel']} />
          </Post>
        </Msg>
      )}

      {/* 8 ── Matty: typing ──────────────────────────────────────────────── */}
      {phase === 8 && <Msg><TypingPost /></Msg>}
          ...
          <Post avatarSrc={avatarMatty} avatarAlt="Matty" username="Matty" timestamp="just now" isBot botLabel="Agent">
            ...
            {(phase === 9 || dismissing === 9) && (
              <div className={widgetCls(9)}>
                <MessageAttachment
                  header={{ title: 'Select Rotation Cadence', chip: { label: 'AWAITING INPUT', variant: 'warning' } }}
                  actions={[
                    { label: 'Confirm', variant: 'primary',   onClick: () => { if (cadence !== null) dismissAndAdvance(9, 10) } },
                    { label: 'Cancel',  variant: 'secondary' },
                  ]}
                >
                  <div style={pickerGroup}>
      ...
      {/* 11 ── Matty: typing ─────────────────────────────────────────────── */}
      {phase === 11 && <Msg><TypingPost /></Msg>}

      {/* 12 ── Matty: summary + confirm ──────────────────────────────────── */}
      {phase >= 12 && (
        <Msg>
          <Post avatarSrc={avatarMatty} avatarAlt="Matty" username="Matty" timestamp="just now" isBot botLabel="Agent">
            <p style={msgText}>Here's what I'll set up — take a look before I make any changes.</p>
            {(phase === 12 || dismissing === 12) && (
              <div className={widgetCls(12)}>
                <MessageAttachment
                  header={{ title: 'Create On-Call Schedule', chip: { label: 'CONFIRMATION NEEDED', variant: 'warning' } }}
                  fields={[
                    ...
                    { label: 'Rotation',   value: selectedNames.length > 0 ? selectedNames.join(', ') : 'Ethan Brooks, Isabella Cruz, Arjun Patel', colSpan: 2 },
                    { label: 'Escalation', value: 'Leila Haddad — Engineering Manager', colSpan: 2 },
                  ]}
                  fieldColumns={2}
                  actions={[
                    { label: 'Confirm', variant: 'primary',   onClick: () => dismissAndAdvance(12, 13) },
                    { label: 'Cancel',  variant: 'secondary' },
                  ]}
                />
              </div>
      ...
      {phase >= 15 && (
        <Msg>
          <Post avatarSrc={avatarMatty} avatarAlt="Matty" username="Matty" timestamp="just now" isBot botLabel="Agent">
            <p style={msgText}>
              All done! The schedule is live and the first shift kicks off Monday. I've sent calendar invites to everyone on the rotation.
            </p>
            <MessageAttachment
              header={{
      ...
      {phase >= 15 && (
        <Msg>
          <div className={styles.replayRow}>
            <button className={styles.replayButton} onClick={restart}>Replay conversation</button>
          </div>
        </Msg>
      )}
